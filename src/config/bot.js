...
export const data = new SlashCommandBuilder()
  .setName("announce")
  .setDescription("Announce a live stream to the server.")
...
export async function execute(interaction, config) {
  try {
    ...
    const link = interaction.options
      .getString("link", true)
      .trim();
    ...
    try {
      streamURL = new URL(link);
    } catch {
      return interaction.reply({
        content: "❌ Please provide a valid stream URL.",
        ephemeral: true,
      });
    }
    ...
    if (!["http:", "https:"].includes(streamURL.protocol)) {
      return interaction.reply({
        content: "❌ The stream link must use HTTP or HTTPS.",
        ephemeral: true,
      });
    }
    ...
    const announceConfig = config?.commands?.announce ?? {};

    const title =
      announceConfig.title ??
      "🔴 LIVE NOW!";

    const description =
      announceConfig.description ??
      "The stream is now live! Come join and show some support! ❤️";

    const buttonText =
      announceConfig.buttonText ??
      "🔴 WATCH STREAM";

    const pingEveryone =
      announceConfig.pingEveryone ?? true;
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(`${platformEmoji} ${title}`)
      .setDescription(
        [
          description,
          "",
          `🎥 **Platform:** ${platform}`,
          `🔗 **Stream:** [Click here to watch](${streamURL.toString()})`,
        ].join("\n")
      )
      .addFields(
        {
      ...
      .setFooter({
        text: "Thanks for supporting the stream! ❤️",
      })
      .setTimestamp();
    ...
    if (announceConfig.channelId) {
      const configuredChannel =
        interaction.guild.channels.cache.get(
          announceConfig.channelId
        );
    ...
    if (!targetChannel) {
      return interaction.reply({
        content:
          "❌ I couldn't find a channel to send the announcement to.",
        ephemeral: true,
      });
    }
