...
export async function execute(interaction, config) {
  ...
  try {
    ...
    return interaction.reply({
      content: "❌ Please provide a valid stream link.",
      ephemeral: true,
    });
  }
  ...
  const embed = new EmbedBuilder()
    .setColor(announceConfig.color)
    .setTitle(announceConfig.title)
    .setDescription(
      `${announceConfig.description}\n\n` +
      `🔗 **Stream:** ${streamURL.toString()}`
    )
    .setFooter({
      text: `Stream announced by ${interaction.user.username}`,
    })
    .setTimestamp();

  const button = new ButtonBuilder()
    .setLabel(announceConfig.buttonText)
    .setStyle(ButtonStyle.Link)
    .setURL(streamURL.toString());

  const row = new ActionRowBuilder().addComponents(button);

  const content = announceConfig.pingEveryone
    ? "@everyone"
    : "";

  const targetChannel = announceConfig.channelId
    ? interaction.guild.channels.cache.get(announceConfig.channelId)
    : interaction.channel;

  if (!targetChannel) {
    return interaction.reply({
      content: "❌ I couldn't find the announcement channel.",
      ephemeral: true,
    });
  }
  ...
  await targetChannel.send({
    ...
    components: [row],
    allowedMentions: {
      parse: announceConfig.pingEveryone ? ["everyone"] : [],
    },
  });

  await interaction.reply({
    content: "✅ Stream announcement sent!",
    ephemeral: true,
  });
}
